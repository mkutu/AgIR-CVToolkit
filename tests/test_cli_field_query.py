...
def sample_db_path() -> Path:
    ...
    db_path = Path("tests/data/db/agir_field_v2_1000.db")
    ...
    def _fake_compose_cfg(config_name: str = "config", overrides: Optional[List[str]] = None):
        cfg = OmegaConf.create({
            "db": {"field": {
                ...
                "table": "field_data",                       # change if your test DB uses another table name
            }},
...
def test_query_with_filters_sort_and_preview(tmp_path):
    ...
    result = runner.invoke(
        ...
        [
            "query",
            "--db", "field",
            "-f", "us_state=NC",                         # adjust to a column that exists in your DB if needed
            "-f", "common_name='barley','hairy vetch'",
...
def test_query_filter_by_estimated_bbox_area_cm2(tmp_path):
    ...
    res = runner.invoke(
        ...
        [
            "query",
            "--db", "field",
            "-f", "ground_cover=='[0.0, 25.0]','[76.0, 100.0]'",
            "--limit", "20",
            "--projection", "image_id,image_url,developed_image_path,ground_cover",
            "--out", "json",
            "--out-path", str(out_path),
        ],
    )
...
def test_query_filter_by_estimated_bbox_area_cm2_between(tmp_path):
    ...
    res = CliRunner().invoke(
        ...
        [
            "query",
            "--db", "field",
            "-f", "flower_fruit_or_seeds==True",
            "--limit", "20",
            "--projection", "developed_image_path,image_id,flower_fruit_or_seeds",
            "--out", "json",
            "--out-path", str(out_path),
        ],
    )
